...
function FormDialog({
  ...
  return <dialog ref={dialog} className="dialog" onCancel={close} onClick={e => {
    ...
    {error && <div role="alert" className="alert error">{hidden ? 'Existe um conflito financeiro. Mostre os valores para consultar os detalhes.' : error}</div>}<div className="dialog-footer"><button type="button" className="button secondary" onClick={close}>Cancelar</button><button className="button primary" disabled={busy}>{busy ? 'Salvando…' : 'Salvar'}<Check size={16} /></button></div>
  </form></dialog>;
}
function Empty({
  icon: Icon = Wallet,
...
function Stat({
  ...
  return <div className={`stat ${highlight ? 'highlight' : ''}`}><div className="stat-top"><span>{label}</span><Icon size={19} /></div><strong className={negative ? 'negative' : ''}><MoneyValue value={value} /></strong><span className="stat-note">{note}</span></div>;
}
function GoalCard({
  goal,
  add,
  ...
  return <div className="goal-card"><div className="goal-head"><span className="goal-symbol"><Target size={19} /></span><span className="tag">{{
          ...
          medium: 'Médio prazo',
          long: 'Longo prazo'
        }[goal.term_type]}</span>{edit && <button className="icon-button" aria-label={`Editar ${goal.name}`} onClick={edit}><Pencil size={15} /></button>}</div><h3>{goal.name}</h3><div className="goal-amount"><strong><MoneyValue value={goal.accumulated} /></strong><span>de <MoneyValue value={goal.target_amount} /></span></div><div className="progress"><span style={{ width: hidden ? "0%" : `${goal.progress}%` }} className={hidden ? "hidden-progress" : ""} /></div><div className="goal-footer"><span>{hidden ? "Valor oculto" : `${Number(goal.progress)}% concluído`}</span><span>{dateLabel(goal.target_date)}</span></div><div className="goal-monthly"><span>{goal.overdue ? 'Prazo encerrado' : 'Aporte necessário'}</span><strong><MoneyValue value={goal.monthly_required} /><small>/mês</small></strong></div>{add && <button className="button secondary full" onClick={add}><Plus size={15} />Registrar aporte</button>}</div>;
}
function PaymentList({
  ...
  return visible.length ? <div className="payment-list">{visible.map(p => <div className="payment-row" key={p.key}><div className={`date-box ${p.status === 'overdue' ? 'late' : p.days_until <= 7 && !['paid', 'forecast', 'registered'].includes(p.status) ? 'soon' : ''}`}><strong>{p.due_date.slice(8)}</strong><span>{MONTHS[Number(p.due_date.slice(5, 7)) - 1]}</span><small>{p.due_date.slice(0, 4)}</small></div><div className="payment-info"><strong>{p.description}</strong><span className={p.status === 'overdue' ? 'negative' : ''}>{p.status === 'paid' ? 'Pago' : p.status === 'registered' ? 'Cobrança registrada' : p.status === 'forecast' ? 'Compromisso previsto' : p.status === 'overdue' ? 'Vencido · pendente' : p.days_until === 0 ? 'Vence hoje' : p.days_until <= 7 ? `Vence em ${p.days_until} dia(s)` : 'A vencer'} · {p.origin}</span><small>Competência {p.month.slice(5, 7)}/{p.month.slice(0, 4)} · vencimento {dateLabel(p.due_date)}</small></div><strong><MoneyValue value={p.amount} /></strong>{pay && p.can_pay && <button className="icon-button pay" title="Registrar pagamento" aria-label={`Marcar ${p.description} como pago`} onClick={() => pay(p)}><Check size={18} /></button>}</div>)}</div> : <Empty icon={CheckCircle2} title="Nenhum pagamento nesta competência" text="Não há obrigações para o mês selecionado." />;
}
function Categories({
  rows
}) {
  const { hidden } = useValueVisibility();
  return rows.length ? <div className="category-content"><div className="donut">{hidden ? <div className="hidden-chart" style={{height: 185, display: "flex", alignItems: "center", justifyContent: "center", color: "#83877c"}}>Valores ocultos</div> : <ResponsiveContainer width="100%" height={185}><PieChart><Pie data={rows.map(r => ({
            ...r,
        ...
        }} /><span>{r.name}</span><strong><MoneyValue value={r.amount} /></strong></div>)}</div></div> : <Empty icon={ChartNoAxesCombined} title="Seus gastos, bem explicados" text="Registre despesas para ver a distribuição por categoria." />;
}
function Evolution({
  rows
}) {
  const { hidden } = useValueVisibility();
  return <div className="evolution">{hidden ? <div className="hidden-chart" style={{height: 230, display: "flex", alignItems: "center", justifyContent: "center", color: "#83877c"}}>Valores ocultos</div> : <ResponsiveContainer width="100%" height={230}><AreaChart data={rows.map(r => ({
        ...r,
...
export default function App() {
  ...
  function planning() {
    return <><div className="planning-layout"><Panel title="Um plano para o seu mês" sub="Entenda para onde vai cada real"><div className="planning-row"><span><span className="plan-icon green"><ArrowDownRight size={18} /></span>Renda do mês</span><strong><MoneyValue value={summary.income} /></strong></div><div className="planning-row"><span><span className="plan-icon"><ArrowUpRight size={18} /></span>{summary.is_forecast ? 'Gastos previstos' : 'Gastos do mês'}</span><strong>− <MoneyValue value={summary.expenses} /></strong></div><div className="planning-row"><span><span className="plan-icon yellow"><Target size={18} /></span>Aportes necessários para metas</span><strong>− <MoneyValue value={summary.goal_required} /></strong></div><div className="planning-total"><span>Dinheiro livre<strong className={Number(summary.free) < 0 ? 'negative' : ''}><MoneyValue value={summary.free} /></strong></span><span className="plan-icon"><Wallet size={25} /></span></div><p className="footnote">Os gastos incluem despesas, parcelas, contas a pagar, faturas e assinaturas previstas uma única vez. Aportes já feitos não são descontados novamente.</p>{summary.is_forecast && !summary.has_income && <p className="forecast-note">Nenhuma receita foi cadastrada para esta competência. O saldo projetado não é uma certeza de déficit.</p>}</Panel><Panel title={summary.feasible ? 'Seu planejamento está equilibrado' : 'Seu plano precisa de atenção'} sub="Renda, gastos e metas na mesma conta"><div className={`planning-status ${summary.feasible ? '' : 'warning'}`}>{summary.feasible ? <CheckCircle2 size={32} /> : <AlertCircle size={32} />}<h3>{summary.feasible ? 'Cabe no seu mês' : 'Vamos ajustar o plano?'}</h3><p>{summary.feasible ? `Depois dos gastos e das metas, sobram $<MoneyValue value={summary.free} /> neste mês.` : `O déficit é de $<MoneyValue value={summary.deficit} />. Revise os gastos ou aumente o prazo das metas. Metas vencidas também precisam de revisão.`}</p></div><div className="planning-mini"><span>Parcelas nesta competência</span><strong><MoneyValue value={summary.installment_total} /></strong></div><div className="planning-mini"><span>Assinaturas previstas</span><strong><MoneyValue value={summary.subscription_projected_total} /></strong></div><button className="button secondary full" onClick={() => add('transactions', {
          kind: 'income'
        })}><Plus size={16} />Registrar receita</button></Panel></div>{summary.forecast && <Panel title="Próximos 12 meses" sub="Parcelas e assinaturas ativas entram na estimativa"><div className="table-scroll"><table><thead><tr><th>Competência</th><th>Parcelas</th><th>Assinaturas previstas</th><th>Gastos previstos</th><th>Receita cadastrada</th></tr></thead><tbody>{summary.forecast.map(row => <tr key={row.month}><td>{MONTHS[Number(row.month.slice(5, 7)) - 1]} / {row.month.slice(0, 4)}</td><td><MoneyValue value={row.installment_total} /></td><td><MoneyValue value={row.subscription_projected_total} /></td><td className="amount"><MoneyValue value={row.expenses} /></td><td className={row.has_income ? 'positive' : 'muted'}>{row.has_income ? money(row.income) : 'Não informada'}</td></tr>)}</tbody></table></div></Panel>}</>;
  }
  const subtitles = {
  ...
  return <div className="app"><aside className={`sidebar ${mobile ? 'open' : ''}`}><a className="brand" href="/" onClick={e => {
    ...
    {error && <div className="alert error" role="alert"><AlertCircle size={18} /><div>{error.includes('fetch') ? 'Não foi possível conectar ao servidor. Verifique se o Django está em execução.' : (hidden ? 'Existe um conflito financeiro. Mostre os valores para consultar os detalhes.' : error)}</div><button className="text-button" onClick={load}>Tentar novamente</button></div>}
    ...
    {loading ? <div className="loading" role="status"><LoaderCircle size={26} className="spin" />Organizando suas finanças…</div> : summary && <>
      ...
      {section === 'accounts' && !detailId && <div className="account-grid">{summary.accounts.map((a, i) => <section className={`account-card ${!a.is_active ? 'archived' : ''}`} key={a.id}><div className="account-top"><span className="bank-icon" style={{
                  background: COLORS[i % COLORS.length]
                }}>{a.name.slice(0, 2).toUpperCase()}</span><span className="tag">{a.is_active ? 'Ativa' : 'Arquivada'}</span>{editButton('accounts', a)}</div><h2>{a.name}</h2><p>{a.institution || 'Conta pessoal'}</p><span className="eyebrow">SALDO INFORMADO</span><strong className="account-balance"><MoneyValue value={a.balance} /></strong><small>Posição: {a.balance_month ? dateLabel(a.balance_month).slice(3) : 'sem registro'}</small><div className="account-bottom">{link('Ver detalhes', `/accounts/${a.id}`)}<button className="text-button muted" onClick={() => archive('accounts', a)}>{a.is_active ? 'Arquivar' : 'Reativar'}</button></div></section>)}<button className="add-card" onClick={() => add('accounts')}><Plus size={26} /><strong>Adicionar conta</strong><span>Conecte seus números, manualmente.</span></button></div>}
